...
def main():
    ...
    #     for i in row:
    #         button = ttk.Button(row_frame, text=f"SSD {i+1}", bootstyle=SECONDARY, width=40, padding=10, font=("Helvetica", 20))
    #         button.grid(row=0, column=row.index(i), padx=10, pady=10, ipady=10, ipadx=20)
    #         buttons.append(button)
    ...
    for row in button_order:
        ...
        row_frame.pack(padx=10, pady=10, expand=True)
        for i in row:
            button = ttk.Button(row_frame, text=f"SSD {i+1}", style=SECONDARY, width=40, padding=10)
            button.grid(row=0, column=row.index(i), padx=10, pady=10, ipady=70, ipadx=40)
            buttons.append(button)
